...
assert _doc_match.padya == "vasantatilakA", f"Sanity check failed: got {_doc_match.padya}"
...
pada_kuru = Pada.Tinanta(dhatu_kr, Prayoga.Kartari, Lakara.Lot, Purusha.Madhyama, Vacana.Eka)
...
tokens = chedaka.run(slp1_pada)
for token in tokens:
    iast_text = transliterate(token.text, Scheme.Slp1, Scheme.Iast)
    iast_lemma = transliterate(token.lemma, Scheme.Slp1, Scheme.Iast)
    print(f"  {iast_text:16} lemma={iast_lemma:12} {token.data}")
